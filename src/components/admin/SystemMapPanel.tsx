  ...
  return [
    {
      id: "discover", number: "01", label: "Discover", summary: "Collect, classify and publish UK contract roles.", nodes: [
        ...
        built({ id: "pipeline", label: "Daily source pipeline", icon: Workflow, description: "Fetches, deduplicates and evaluates IR35 evidence during the daily hosted refresh.", input: "Provider and ATS listings", output: "Fresh deduplicated jobs", connectsTo: ["Supabase data", "Contract search"], failure: "A provider fails, a source changes format or the schedule does not run.", action: "Inspect Pipeline runs and rerun the source refresh." }),
    ...
    },
    {
      id: "prepare", number: "02", label: "Prepare", summary: "Build a truthful, role-specific application packet.", nodes: [
        built({ id: "profile", label: "Contractor profile", icon: UserRound, description: "Holds the facts the runner is allowed to use.", input: "User-approved identity and work details", output: "Verified application facts", connectsTo: ["Resume analysis", "Application runner"], failure: "A required fact is missing or has not been approved.", action: "Ask the contractor only for the missing answer." }),
        built({ id: "cv", label: "Resume analysis", icon: FileCheck2, description: "Scores role evidence and prepares truth-preserving edits.", input: "Original Resume and job description", output: "Approved role-specific Resume", connectsTo: ["AI tailoring", "Approved packet"], failure: "The Resume cannot be parsed or evidence is too limited.", action: "Keep the original content and request better source evidence." }),
        externalNode(integrations, "ai_tailoring", "OpenRouter tailoring", { icon: Bot, description: "Improves unfamiliar wording and field mapping without inventing experience.", input: "Role, Resume evidence and unknown field labels", output: "Suggested edits and field mappings", connectsTo: ["Approved packet", "Application runner"], failure: "Invalid key, provider outage or model response rejected by validation.", action: integration(integrations, "ai_tailoring")?.nextStep ?? "Check OpenRouter." }),
        built({ id: "packet", label: "Approved packet", icon: ShieldCheck, description: "Freezes the exact Resume, cover letter and answers approved by the contractor.", input: "Reviewed application materials", output: "Submission-ready packet", connectsTo: ["Application runner"], failure: "Final approval is incomplete or materials changed after approval.", action: "Return the application to the review step." }),
      ],
    },
    {
      id: "apply", number: "03", label: "Apply", summary: "Complete the employer form and retain proof of the result.", nodes: [
        built({ id: "auto_apply_schedule", label: "Daily Auto Apply", icon: Bot, description: "Runs after the daily source refresh, rotates fairly through enabled contractor accounts and starts up to each account's daily allowance.", input: "Complete profile, approved preferences and current contract matches", output: "Fair per-account application queue", connectsTo: ["Application orchestration"], failure: "A profile is incomplete, the daily allowance is exhausted or no eligible contract matches.", action: "Inspect the scheduled Auto Apply audit, then show the contractor the exact missing profile answer or matching reason." }),
    ...
    },
    {
      id: "operate", number: "05", label: "Operate", summary: "Secure, observe and administer the platform.", nodes: [
        ...
        built({ id: "observability", label: "Pipeline and audit logs", icon: Gauge, description: "Records source runs, scheduled Auto Apply batches, campaigns, employer connections, email delivery and runner tests.", input: "Operational events", output: "Traceable health evidence", connectsTo: ["Admin control"], failure: "A workflow exits before its audit event is saved.", action: "Treat the missing receipt as a failure and investigate the originating service." }),
      ],
    },
  ];
}
  ...
  return (
    <div className="mt-7 space-y-5">
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-slate-950 text-white shadow-sm">
        ...
        <div className="border-t border-white/10 bg-white/[0.04] px-5 py-3 text-xs leading-5 text-slate-300 sm:px-6">This controlled production test uses the real hosted runner, Resume upload, multi-step form and confirmation detector. It does not contact an employer.</div>
      </section>
              ...
              return (
                <article key={run.id} className="grid gap-3 px-5 py-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:px-6">
                  <div className="min-w-0">
                    ...
                    {run.diagnostic ? (
                      <details className="mt-3 rounded-xl border border-slate-200 bg-slate-50 p-3 text-[11px] text-slate-700">
                        <summary className="cursor-pointer font-semibold text-slate-900">Protected stopped-page snapshot</summary>
