...
export function ApplicationStudio({ job }: { job: JobDetail }) {
  ...
  const requiredQuestions = application?.questions.filter((question) => question.required) ?? [];
  const reusableQuestions = application?.questions.filter((question) => !question.required) ?? [];
  ...
  const runAiTailoring = async () => {
    ...
    try {
      ...
      const suggestionIds = payload.result.suggestions.map((suggestion) => suggestion.id);
      setAiResult(payload.result);
      setSelectedSuggestionIds(suggestionIds);
      setUseAiCoverLetter(applicationPreferences.generateCoverLetter && Boolean(payload.result.coverLetter));
      if (applicationPreferences.autoApproveSafeEdits && suggestionIds.length > 0) {
        const next = mergeTailoringResult(application, payload.result);
        const updated = { ...next, status: "needs_review" as const, truthApproved: false, materialsApproved: false, submissionApproved: false, updatedAt: new Date().toISOString() };
        setApplication(updated);
        persistApplication(updated);
        setNotice(`Tailoring refreshed and ${suggestionIds.length} verified CV improvement${suggestionIds.length === 1 ? " was" : "s were"} applied. Review the final CV before approving it.`);
      } else {
        setNotice(`Tailoring refreshed. ${suggestionIds.length} verified CV improvement${suggestionIds.length === 1 ? " is" : "s are"} selected for your review.`);
      }
    } catch (caught) {
      const message = caught instanceof Error ? caught.message : "";
      if (/session|sign in/i.test(message)) setError(message);
      else setError(message || "The wording refresh could not finish. Your existing CV has not been changed. Try again in a moment.");
    } finally {
      setBusy(null);
    }
  };
  ...
  const submitApprovedApplication = async () => {
    ...
    if (!approvalsComplete) {
      setError("Confirm that the final application is accurate, truthful and ready to submit.");
      return;
    }
    setBusy("submit");
    submissionStatusFailures.current = 0;
    ...
    try {
      ...
      if (response.status === 202 && payload.state) {
        ...
        const submissionStarted: ApplicationRecord = {
          ...ready,
          events: ready.events.some((event) => event.label === "Application submission started")
            ? ready.events
            : [...ready.events, { id: newWorkspaceId(), applicationId: ready.id, type: "status_changed", label: "Application submission started", createdAt: new Date().toISOString() }],
          updatedAt: new Date().toISOString(),
        };
          ...
          {!application ? (
            ...
            <>
              <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-card sm:p-6">
                ...
                <div className="mt-5 grid gap-4 sm:grid-cols-2"><div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4"><p className="text-xs font-bold uppercase tracking-wide text-emerald-800">Evidence found</p><div className="mt-3 flex flex-wrap gap-2">{application.matchedKeywords.length ? application.matchedKeywords.map((term) => <span key={term} className="rounded-lg bg-white px-2 py-1 text-xs font-medium text-emerald-900">{term}</span>) : <span className="text-sm text-emerald-900">No strong keyword matches yet.</span>}</div></div><div className="rounded-2xl border border-amber-200 bg-amber-50 p-4"><p className="text-xs font-bold uppercase tracking-wide text-amber-800">Missing keywords, not assumed</p><div className="mt-3 flex flex-wrap gap-2">{application.missingKeywords.length ? application.missingKeywords.map((term) => <span key={term} className="rounded-lg bg-white px-2 py-1 text-xs font-medium text-amber-950">{term}</span>) : <span className="text-sm text-amber-950">No material gaps detected.</span>}</div></div></div>
              </section>
              ...
              <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-card">
                ...
                <div className="p-5 sm:p-6">
                  ...
                  <p className="mt-3 text-xs leading-5 text-slate-500">The refresh rebuilds the profile, skills and relevant experience wording from verified CV evidence. <Link href="/ai-disclosure" target="_blank" className="font-semibold text-brand-700 underline">How tailoring works</Link></p>
              ...
              <details open className="group rounded-3xl border border-slate-200 bg-white shadow-card">
                ...
                <div className="border-t border-slate-200 p-5 sm:p-6"><p className="text-sm leading-6 text-slate-600">Confirm the essential answers for this application. IR35Careers also keeps a broader answer bank ready for common ATS forms and pauses when an employer asks something new.</p><div className="mt-4 space-y-3">{requiredQuestions.map((question) => <div key={question.id} className={`rounded-2xl border p-4 ${question.reviewed ? "border-emerald-200 bg-emerald-50/40" : "border-amber-200 bg-amber-50/40"}`}><label htmlFor={`question-${question.id}`} className="text-sm font-semibold text-slate-950">{question.label}</label><input id={`question-${question.id}`} value={question.answer} onChange={(event) => updateApplication((current) => ({ ...current, questions: current.questions.map((item) => item.id === question.id ? { ...item, answer: event.target.value, reviewed: false } : item), status: "needs_review", submissionApproved: false }))} className="ir35-focus mt-2 min-h-11 w-full rounded-xl border border-slate-300 bg-white px-3 text-sm" /><label className="mt-2 flex min-h-10 cursor-pointer items-center gap-3 text-sm text-slate-700"><input type="checkbox" checked={question.reviewed} onChange={(event) => updateApplication((current) => ({ ...current, questions: current.questions.map((item) => item.id === question.id ? { ...item, reviewed: event.target.checked } : item), status: "needs_review", submissionApproved: false }))} className="h-5 w-5 accent-emerald-700" /> I confirm this answer is accurate</label></div>)}</div>{reusableQuestions.length > 0 && <details className="group mt-4 rounded-2xl border border-slate-200 bg-slate-50"><summary className="ir35-focus flex min-h-12 cursor-pointer list-none items-center justify-between gap-3 px-4 text-sm font-semibold text-slate-800">Reusable ATS answer bank <span className="flex items-center gap-2 text-xs text-slate-500">{reusableQuestions.length} answers <ChevronDown className="transition-transform group-open:rotate-180" size={16} /></span></summary><div className="space-y-3 border-t border-slate-200 p-4">{reusableQuestions.map((question) => <div key={question.id} className={`rounded-xl border bg-white p-4 ${question.reviewed ? "border-emerald-200" : "border-slate-200"}`}><label htmlFor={`question-${question.id}`} className="text-sm font-semibold text-slate-950">{question.label}</label><input id={`question-${question.id}`} value={question.answer} onChange={(event) => updateApplication((current) => ({ ...current, questions: current.questions.map((item) => item.id === question.id ? { ...item, answer: event.target.value, reviewed: false } : item), status: "needs_review", submissionApproved: false }))} className="ir35-focus mt-2 min-h-11 w-full rounded-xl border border-slate-300 px-3 text-sm" /><label className="mt-2 flex min-h-10 cursor-pointer items-center gap-3 text-sm text-slate-700"><input type="checkbox" checked={question.reviewed} onChange={(event) => updateApplication((current) => ({ ...current, questions: current.questions.map((item) => item.id === question.id ? { ...item, reviewed: event.target.checked } : item), status: "needs_review", submissionApproved: false }))} className="h-5 w-5 accent-emerald-700" /> Confirm for future applications</label></div>)}</div></details>}</div>
              </details>

              <section id="final-application-approval" className="scroll-mt-24 rounded-3xl border border-slate-800 bg-slate-950 p-5 text-white shadow-floating sm:p-6">
