          ...
          {!application ? (
            ...
            <>
              ...
              <section id="final-application-approval" className="scroll-mt-24 rounded-3xl border border-slate-800 bg-slate-950 p-5 text-white shadow-floating sm:p-6">
                ...
                {!submitted && <p className="mt-3 text-xs leading-5 text-slate-300">{!answersReviewed ? "Confirm every required answer before applying." : !approvalsComplete ? "Tick the final approval above to continue." : submissionConnection === "loading" ? "Checking secure application access." : submissionConnection === "gated" ? "Application access could not be confirmed. Check again or save your reviewed application." : "Ready. IR35Careers will submit the approved materials and wait for employer confirmation."}</p>}
                <div className="mt-5 flex flex-col gap-3 sm:flex-row">{submitted ? <span className="inline-flex min-h-12 items-center gap-2 rounded-xl bg-emerald-400/15 px-4 text-sm font-bold text-emerald-200"><CheckCircle2 size={17} /> Application submitted</span> : <><button type="button" onClick={() => void submitApprovedApplication()} disabled={busy !== null || submissionConnection !== "connected"} className="ir35-focus inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-emerald-400 px-6 text-sm font-bold text-emerald-950 hover:bg-emerald-300 disabled:cursor-not-allowed disabled:opacity-50">{busy === "submit" || submissionConnection === "loading" ? <Loader2 className="animate-spin" size={16} /> : <Send size={16} />} Approve and apply now</button>{submissionConnection === "gated" && <button type="button" onClick={() => { setSubmissionConnection("loading"); setSubmissionStatusRefresh((current) => current + 1); }} disabled={busy !== null} className="ir35-focus inline-flex min-h-12 items-center justify-center gap-2 rounded-xl border border-white/20 bg-white/5 px-5 text-sm font-bold text-white hover:bg-white/10"><RotateCcw size={16} /> Check again</button>}{submissionConnection === "gated" && <button type="button" onClick={() => void saveReviewedPacket()} disabled={busy !== null || !answersReviewed || !approvalsComplete} className="ir35-focus inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-white px-5 text-sm font-bold text-slate-950 hover:bg-slate-100 disabled:opacity-40">{busy === "save" ? <Loader2 className="animate-spin" size={16} /> : <Save size={16} />} Save application</button>}</>}</div>
              </section>
