...
export function AutomationSettings() {
  ...
  return (
    <WorkspacePage eyebrow="Controlled automation" title="Set the rules once. Review every packet." description="Monitor live contracts, build a focused review queue and apply from IR35Careers after final approval. Role discovery and employer submission remain separate, visible controls.">
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_380px]">
        <div className="space-y-6">
          ...
          <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-card sm:p-6"><h2 className="font-semibold">Non-negotiable controls</h2><div className="mt-4 grid gap-3 sm:grid-cols-2"><div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4"><ShieldCheck className="text-emerald-700" /><p className="mt-3 text-sm font-semibold text-emerald-950">Monitoring is review-only</p><p className="mt-1 text-xs leading-5 text-emerald-900">Matching roles enter a queue. Monitoring never submits an ATS form or sends an email.</p></div><div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4"><Eye className="text-emerald-700" /><p className="mt-3 text-sm font-semibold text-emerald-950">Human approval required</p><p className="mt-1 text-xs leading-5 text-emerald-900">Review the exact CV, letter and answers, then apply from the final screen when the employer connection is verified.</p></div></div></section>
        ...
        <aside className="space-y-5 xl:sticky xl:top-24 xl:h-max">
          ...
          <section className={`rounded-3xl border p-5 shadow-card ${submissionState === "connected" ? "border-emerald-200 bg-emerald-50" : "border-amber-200 bg-amber-50"}`}><PlugZap size={20} className={submissionState === "connected" ? "text-emerald-700" : "text-amber-800"} /><p className={`mt-3 text-xs font-bold uppercase tracking-wide ${submissionState === "connected" ? "text-emerald-700" : "text-amber-800"}`}>Direct-apply connection</p><h2 className={`mt-2 font-semibold ${submissionState === "connected" ? "text-emerald-950" : "text-amber-950"}`}>{submissionState === "loading" ? "Checking employer connections…" : submissionState === "connected" ? "Direct apply is connected" : "Employer connection required"}</h2><p className={`mt-2 text-sm leading-6 ${submissionState === "connected" ? "text-emerald-900" : "text-amber-900"}`}>{submissionState === "connected" ? "Approved packets can be submitted from their final review screen without leaving IR35Careers, with duplicate protection and an ATS receipt." : "Rules can monitor and prepare now. Unsupported roles remain queued until an authorised employer or ATS gateway is connected."}</p></section>
