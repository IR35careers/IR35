...
async function authenticatedPost(path: string, idempotencyKey?: string, requestBody?: object): Promise<{ url?: string; error?: string }> {
  ...
  return response.ok ? responseBody : { error: responseBody.error || "The billing provider did not respond." };
}
...
export function BillingManager() {
  ...
  return <WorkspacePage eyebrow="Plans and billing" title="Choose capacity, not hidden automation" description="Checkout, plan changes and cancellations use a hosted payment-provider flow. Provider access stays disabled until every production credential, price and signed webhook is configured.">
    ...
    <div className="grid gap-5 lg:grid-cols-2">
      ...
      <article className={`rounded-3xl border p-6 shadow-card ${active ? "border-brand-300 bg-brand-50" : "border-slate-200 bg-white"}`}><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-bold uppercase tracking-wide text-brand-700">Contractor Pro</p><p className="mt-3 text-3xl font-bold text-slate-950">{checkoutReady ? priceLabel : "Not on sale"}</p><p className="mt-2 text-sm text-slate-600">Pricing and renewal terms are shown again in hosted checkout.</p></div><ShieldCheck className="text-brand-700" /></div><ul className="mt-6 space-y-3">{PRO_FEATURES.map((feature) => <li key={feature} className="flex items-center gap-2 text-sm text-slate-700"><Check className="text-brand-700" size={16} />{feature}</li>)}</ul>{needsPaymentReview && <p className="mt-5 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-950">A payment needs attention. Open billing management to review it securely.</p>}{checkoutReady && !active && !sandbox && !needsPaymentReview && <label className="mt-5 flex cursor-pointer items-start gap-3 rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm leading-6 text-slate-700"><input type="checkbox" checked={checkoutConsent} onChange={(event) => setCheckoutConsent(event.target.checked)} className="mt-1 h-5 w-5 shrink-0 accent-emerald-700" /><span>I accept the <Link href="/terms" className="font-semibold text-brand-700 underline">Terms</Link> and <Link href="/billing-policy" className="font-semibold text-brand-700 underline">Billing Policy</Link>, and request immediate plan access. I understand the stated cooling-off and possible proportionate-charge terms.</span></label>}{checking ? <div className="mt-6 flex min-h-12 items-center justify-center text-slate-500"><Loader2 className="animate-spin" size={18} /><span className="sr-only">Checking billing provider</span></div> : active || sandbox || needsPaymentReview ? <button type="button" onClick={() => void openProvider("portal")} disabled={!managementReady || busy !== null} className="ir35-focus mt-6 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-slate-950 px-4 text-sm font-bold text-white disabled:bg-slate-200 disabled:text-slate-500">{busy === "portal" ? <Loader2 className="animate-spin" size={16} /> : <ExternalLink size={16} />} {managementReady ? "Manage billing" : "Billing management unavailable"}</button> : <button type="button" onClick={() => void openProvider("checkout")} disabled={!checkoutReady || !checkoutConsent || busy !== null} className="ir35-focus mt-6 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-brand-600 px-4 text-sm font-bold text-white disabled:bg-slate-200 disabled:text-slate-500">{busy === "checkout" ? <Loader2 className="animate-spin" size={16} /> : checkoutReady ? <CreditCard size={16} /> : <LockKeyhole size={16} />}{checkoutReady ? "Continue to secure checkout" : "Checkout not connected"}</button>}</article>
    </div>
    <div className="mt-6 rounded-3xl border border-amber-200 bg-amber-50 p-5"><p className="font-semibold text-amber-950">Production safeguards</p><p className="mt-1 text-sm leading-6 text-amber-900">Checkout uses an authenticated server request and an idempotency key. Entitlements change only after a signed Stripe webhook is reconciled. Sandbox events never unlock a production Pro plan. Plan management and cancellation open in Stripe&apos;s short-lived customer portal.</p></div>
  </WorkspacePage>;
}
