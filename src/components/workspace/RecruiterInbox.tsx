...
export function RecruiterInbox() {
  ...
  return (
    <WorkspacePage eyebrow="Recruiter inbox" title="Responses linked to the right role" description="A private application address can receive recruiter updates, classify them, and connect each message to its application. This local preview uses fictional messages and sends nothing.">
      <section className="grid gap-4 lg:grid-cols-[1fr_360px]">
        <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-card sm:p-6">
          ...
          <div className="mt-5 flex flex-col gap-4 border-t border-slate-100 pt-5 sm:flex-row sm:items-center sm:justify-between">
            ...
            <button type="button" role="switch" aria-checked={workspace.inbox.forwardingEnabled} onClick={toggleForwarding} className={`ir35-focus relative h-11 w-16 rounded-full border transition-colors ${workspace.inbox.forwardingEnabled ? "border-brand-600 bg-brand-600" : "border-slate-300 bg-slate-200"}`}><span className={`absolute top-1.5 h-7 w-7 rounded-full bg-white shadow transition-transform ${workspace.inbox.forwardingEnabled ? "translate-x-7" : "translate-x-1"}`} /><span className="sr-only">Toggle message forwarding</span></button>
          </div>
        </div>
      ...
      <div className="mt-6 flex flex-wrap gap-2" aria-label="Inbox filters">{FILTERS.map((item) => <button key={item.id} type="button" onClick={() => setFilter(item.id)} className={`ir35-focus min-h-11 rounded-xl border px-4 text-sm font-semibold ${filter === item.id ? "border-slate-950 bg-slate-950 text-white" : "border-slate-300 bg-white text-slate-600 hover:border-slate-400"}`}>{item.label}</button>)}</div>
