  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/integrations/email/send", {
        method: "POST",
        headers: { authorization: `Bearer ${token}`, "Content-Type": "application/json", "X-Idempotency-Key": replyRequestKey },
        body: JSON.stringify({ replyToMessageId: replyMessage?.id, message: composeMessage }),
      });
    ...
    <WorkspacePage eyebrow="Recruiter inbox" title="Your application messages" description="Keep confirmations, recruiter replies, assessments and interviews linked to the right application.">
      ...
      <section className="mt-5 rounded-3xl border border-slate-200 bg-white p-4 shadow-card">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-wrap gap-2"><button type="button" onClick={markAllRead} disabled={unread === 0} className="ir35-focus inline-flex min-h-11 items-center gap-2 rounded-xl border border-slate-300 px-4 text-sm font-semibold text-slate-700 disabled:opacity-40"><CheckCheck size={16} /> Mark all read</button></div>
        </div>
      ...
      {composeOpen && replyMessage && <div className="fixed inset-0 z-[80] flex items-end justify-center bg-slate-950/45 p-3 sm:items-center sm:p-6" role="presentation"><section role="dialog" aria-modal="true" aria-labelledby="compose-title" className="w-full max-w-2xl rounded-3xl border border-slate-200 bg-white p-5 shadow-2xl sm:p-6"><div className="flex items-start justify-between gap-4"><div><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-brand-700">Private application email</p><h2 id="compose-title" className="mt-1 text-xl font-semibold text-slate-950">Reply to recruiter</h2><p className="mt-1 break-all text-xs text-slate-500">To {replyMessage.from}</p><p className="mt-1 text-xs text-slate-500">Re: {replyMessage.subject.replace(/^(?:\s*re\s*:\s*)+/i, "")}</p></div><button type="button" onClick={() => setComposeOpen(false)} className="ir35-focus flex h-10 w-10 items-center justify-center rounded-xl bg-slate-100 text-slate-600" aria-label="Close composer"><X size={18} /></button></div><div className="mt-5 grid gap-4"><label className="text-sm font-semibold text-slate-800">Message<textarea value={composeMessage} onChange={(event) => setComposeMessage(event.target.value)} rows={9} maxLength={40_000} className="ir35-focus mt-2 w-full resize-y rounded-xl border border-slate-300 p-3 font-normal leading-6" /></label></div><div className="mt-5 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><button type="button" onClick={() => setComposeOpen(false)} className="ir35-focus min-h-11 rounded-xl border border-slate-300 px-4 text-sm font-semibold text-slate-700">Cancel</button><button type="button" onClick={() => void sendMessage()} disabled={sending || !replyRequestKey || !composeMessage.trim()} className="ir35-focus inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-brand-700 px-5 text-sm font-bold text-white hover:bg-brand-800 disabled:opacity-40">{sending ? <Loader2 className="animate-spin" size={16} /> : <Send size={16} />} Send reply</button></div></section></div>}
    </WorkspacePage>
  );
}
