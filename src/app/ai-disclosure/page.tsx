...
const sections = [
  ...
  { id: "scores", label: "Resume and match scores" },
...
export default function AiDisclosurePage() {
  return <LegalDocument eyebrow="Product transparency" title="AI and Automation Disclosure" summary="What IR35Careers calculates automatically, what it does not do, and where a contractor must remain in control." sections={sections}>
    <section aria-labelledby="current"><h2 id="current">What is used today</h2><p className="mt-3">The baseline role-match score, missing-keyword detection and recruiter-message classification use published deterministic rules. When the optional AI connection is enabled, a signed-in user may separately request evidence-grounded Resume edits and a cover-letter draft. The interface labels that request and does not run it silently.</p><LegalCallout><strong>No automated score decides whether you receive an interview, engagement or other opportunity.</strong> Employers and recruiters make their own decisions independently of IR35Careers.</LegalCallout></section>
    <section aria-labelledby="scores"><h2 id="scores">Resume and match scores</h2><p className="mt-3">Resume Studio compares role terms with the text you provide, evidence-oriented wording, role relevance and ATS readability. The full weighted rubric appears beside the score. A missing term is never treated as experience, and it enters a version only after you confirm it is true.</p></section>
    <section aria-labelledby="automation"><h2 id="automation">Application automation</h2><p className="mt-3">After you approve the final application, the IR35Careers server runner can open the employer portal, detect common application fields, upload the approved Resume, enter saved facts and submit the form. That approval also authorises ordinary employer-account creation, required application terms and email verification for the application you selected. No browser extension is required. OpenRouter may be used to map an unfamiliar field label to the name of an existing saved fact. It does not receive the fact value for this mapping and cannot invent an answer. New legal or personal questions, CAPTCHA, multi-factor authentication and identity checks remain under your control.</p></section>
    <section aria-labelledby="provider"><h2 id="provider">Optional AI tailoring</h2><p className="mt-3">IR35Careers can use OpenRouter as an optional processor for a tailoring request you start. Before transmission, direct email addresses, phone numbers and common profile URLs are redacted. The request asks for zero-data-retention routing and denies providers that require data collection. The selected model provider may still process the remaining Resume evidence and job description outside the UK under its own security and subprocessor terms.</p><ul className="mt-4"><li>Structured output and exact-source evidence checks reject unsupported edits.</li><li>New numbers are rejected unless the source evidence already contains them.</li><li>You choose each edit; an AI response never overwrites the source Resume automatically.</li><li>If the provider fails or is unavailable, the original Resume remains unchanged and the deterministic score still works.</li></ul></section>
