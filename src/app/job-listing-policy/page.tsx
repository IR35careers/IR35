...
export default function JobListingPolicyPage() {
  return (
    <LegalDocument eyebrow="Transparency" title="Job Listing and IR35 Information Policy" summary="How contract opportunities are sourced, cleaned and labelled—and what you should verify before applying." sections={[...sections]}>
      ...
      <section aria-labelledby="classification"><h2 id="classification">IR35 classification</h2><LegalCallout><strong>“Outside”, “Inside” and “TBC” are evidence labels, not legal determinations.</strong> High confidence means the status appears in the title; medium means it appears in the listing text; TBC means no explicit status was found.</LegalCallout><p className="mt-3">We do not infer Outside IR35 merely from words such as contract, limited company or day rate. The end client is responsible for a status determination where the off-payroll rules require one, and actual working practices remain important.</p></section>
      <section aria-labelledby="rates"><h2 id="rates">Rates, skills and workplace</h2><p className="mt-3">Rates are parsed from the source text and displayed using the most plausible unit—daily, hourly or annual. Implausible values are withheld or reclassified rather than presented as fact. Skills and remote/hybrid/on-site labels are extracted and normalised for comparison; they may not capture every requirement or attendance detail.</p></section>
      <section aria-labelledby="apply"><h2 id="apply">Applications, corrections and removals</h2><p className="mt-3">IR35Careers sends you to the original source to apply. We do not submit an application without your explicit action, and preparation tools do not guarantee that an advertiser receives anything.</p><p className="mt-3">Use <Link href="/contact">Contact</Link> to report an expired role, incorrect status/rate, duplicate, unsafe destination or rights concern. Include the IR35Careers URL and a short explanation so it can be reviewed efficiently.</p></section>
    </LegalDocument>
  );
}
