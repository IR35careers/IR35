      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(420px,0.8fr)]">
        <section className="scroll-mt-24 rounded-3xl border border-slate-200 bg-white p-5 shadow-card sm:p-6">
          ...
          {showContactForm && <div className="mt-5 grid gap-3 rounded-2xl bg-slate-50 p-4 sm:grid-cols-2">
            ...
            <div className="flex gap-2 sm:col-span-2"><button type="button" onClick={addContact} disabled={!contactDraft.name.trim()} className="ir35-focus min-h-11 rounded-xl bg-slate-950 px-4 text-sm font-bold text-white disabled:opacity-40">Save contact</button><button type="button" onClick={() => setShowContactForm(false)} className="ir35-focus min-h-11 rounded-xl border border-slate-300 bg-white px-4 text-sm font-bold">Cancel</button></div>
          </div>}
