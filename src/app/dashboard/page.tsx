...
function ScoreRing({ score }: { score: number }) {
  ...
  const color = score >= 75 ? "#16a34a" : score >= 50 ? "#4ade80" : "#64748b";
  return (
          ...
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-1.5 text-sm text-slate-500">
                {profile?.cv_filename ? (
                  ...
                  <Link href="/onboarding" className="text-green-700 underline-offset-4 hover:underline">
            ...
            <form onSubmit={onSearch} className="relative w-full lg:w-[420px]">
              ...
              <input
                ...
                className="w-full rounded-xl border border-slate-300 bg-slate-100 py-3 pl-10 pr-24 text-sm placeholder:text-slate-400 focus:border-slate-400 focus:outline-none focus-visible:ring-2 focus-visible:ring-green-500/50"
              />
              <button
                type="submit"
                className="absolute right-1.5 top-1/2 -translate-y-1/2 rounded-lg bg-slate-900 px-3.5 py-1.5 text-xs font-semibold text-white transition-opacity hover:opacity-90"
              >
                Search
              </button>
            </form>
              ...
              const inner = (
                <div className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white p-4 transition-colors hover:border-slate-300">
                  <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-slate-100">
                    <stat.icon size={16} className={stat.warn ? "text-amber-600" : "text-green-700"} />
                  </div>
          ...
          <section className="mt-9">
            <div className="flex items-center justify-between">
              <h2 className="flex items-center gap-2 text-sm font-medium uppercase tracking-wide text-slate-500">
                <Sparkles size={14} className="text-green-700" /> Top matches for you
              </h2>
            ...
            {profile && profile.skills.length === 0 ? (
              <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-8 text-center">
                ...
                <Link href="/onboarding" className="mt-3 inline-flex items-center gap-2 rounded-xl bg-slate-900 px-5 py-2.5 text-sm font-semibold text-white transition-opacity hover:opacity-90">
                    ...
                    >
                      <span
                        className={`absolute inset-y-0 left-0 w-[3px] ${
                          job.ir35_status === "outside" ? "bg-green-500" : job.ir35_status === "inside" ? "bg-rose-500" : "bg-slate-200"
                        }`}
                        aria-hidden
                      />
