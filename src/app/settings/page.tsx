      ...
      <main className="mx-auto max-w-[1600px] px-4 py-8 sm:px-6">
        <div className="grid gap-6 lg:grid-cols-[240px_1fr_320px]">
          ...
          <div className="min-w-0 space-y-6">
            ...
            <section className="rounded-2xl border border-slate-200 bg-white p-6">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><h2 className="flex items-center gap-2 text-sm font-semibold uppercase tracking-wide text-slate-500"><SlidersHorizontal size={15} /> Application defaults</h2><p className="mt-2 text-sm leading-6 text-slate-600">Control how applications move, how Resume improvements are prepared and when IR35Careers pauses for you.</p></div><Link href="/profile#apply-settings" className="ir35-focus inline-flex min-h-10 shrink-0 items-center justify-center rounded-xl bg-brand-700 px-4 text-sm font-bold text-white hover:bg-brand-800">Edit apply settings</Link></div>
              <dl className="mt-5 grid gap-3 sm:grid-cols-2"><div className="rounded-xl bg-slate-50 p-4"><dt className="text-[10px] font-bold uppercase tracking-wide text-slate-500">Application mode</dt><dd className="mt-1 text-sm font-semibold capitalize text-slate-900">{applyPreferences?.applicationMode ?? "automatic"}</dd></div><div className="rounded-xl bg-slate-50 p-4"><dt className="text-[10px] font-bold uppercase tracking-wide text-slate-500">Resume optimisation</dt><dd className="mt-1 text-sm font-semibold capitalize text-slate-900">{applyPreferences?.resumeOptimisation ?? "honest"}</dd></div><div className="rounded-xl bg-slate-50 p-4"><dt className="text-[10px] font-bold uppercase tracking-wide text-slate-500">Safe edits</dt><dd className="mt-1 text-sm font-semibold text-slate-900">{applyPreferences?.autoApproveSafeEdits ? "Apply automatically" : "Review each suggestion"}</dd></div><div className="rounded-xl bg-slate-50 p-4"><dt className="text-[10px] font-bold uppercase tracking-wide text-slate-500">Cover letter</dt><dd className="mt-1 text-sm font-semibold text-slate-900">{applyPreferences?.generateCoverLetter === false ? "Use saved letter only" : "Prepare when required"}</dd></div></dl>
            </section>
