...
The welcome message is a service email, not a marketing subscription. It is sent only to the authenticated, confirmed address and uses server-controlled Supabase account metadata plus a Resend idempotency key so it is delivered once per account. Customers cannot edit the server-controlled delivery marker.
...
2. Keep Resend click and open tracking disabled for authentication messages. Rewriting a confirmation URL can interfere with the Supabase verification link.
3. In Vercel, set server-only production variables:
   - `RESEND_API_KEY` — a Resend key with sending permission;
   - `EMAIL_FROM=IR35Careers <welcome@mail.ir35careers.com>`;
...
4. Confirm exactly one branded welcome email arrives from `welcome@mail.ir35careers.com`.
...
7. Check the Resend delivery log and the user's server-controlled welcome-delivery metadata without exposing message content or provider secrets.
